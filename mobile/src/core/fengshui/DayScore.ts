...
import type { DayFengShuiData, DayScore as DayScoreResult, DayQuality } from '../../data/types/FengShuiData';
...
export class DayScore {
  ...
  static getQuality(score: number): DayQuality {
    if (score >= 80) {
      return 'excellent' as DayQuality;
    } else if (score >= 65) {
      return 'good' as DayQuality;
    } else if (score >= 50) {
      return 'normal' as DayQuality;
    } else if (score >= 35) {
      return 'bad' as DayQuality;
    } else {
      return 'very_bad' as DayQuality;
    }
  }
